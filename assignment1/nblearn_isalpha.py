...
def files2dict(files, dict):
    ...
    for file in files:
        ...
        for line in iter(f):
            ...
            for token in tokens:
                # only consider the token with alphabetic characters
...
def read_data(dir='./Spam or Ham/train', portion=1):
    ...
    # remove token appears only once
    threshold = 2
    for token, frequency in spam.copy().items():
        if frequency <= threshold:
            spam.pop(token)
    for token, frequency in ham.copy().items():
        if frequency <= threshold:
            ham.pop(token)
